...
4. **Declarative configuration**: TOML configuration and a concept of profiles, which are the Lego bricks
   building the sandbox itself. Additionally it tries to give user all information about how sandbox is going
   to be build.
6. **Avoids supply chain attacks**: Written in Go (with string nocgo policy) with a minimal set of 3rd party
   dependencies.
7. **Podman inside**: Runs own `podman` engine allowing docker-based workflows inside.
8. **HTTP Doors**: Can expose HTTP ports from a sandbox on demand. Uses a systemd activation protocol, so
   sandboxed process can NEVER decide to do so.

> **Alpha state**
> The work is focused towards finalizing the internal architecture, the profile model.
> What IS NOT STABLE is the command line interface, the configuration options and so.
> So BEWARE: architecture, options, CLI and configuration may and will be changed.
> Use at your own discretion.

> **LLM generated code**
> Majority of the code has been generated by LLM agents.
> Feel free to not use the tool in case you don't like the way it was written.
> AI agents (redteam, policy, sandbox-tester) regularly try to escape the sandbox closing
> the doors once they're discovered.
...
# ~/.ssh and so is NOT ACCESSIBLE by default
...
🔒 snug:~/src/myproject$ curl https://example.com
...
`snug` itself is based on a simple model - ie share nothing. Then define named
minimal _holes_ into the sandbox, which will make it more practical than empty
one. And this is called _a profile_.

The idea is that profiles are tiny Lego bricks used to build a bigger and
better sandbox by combining those. This is an example
...
> **Alpha state
> A lot of details may and will change, be ready that your configuration won't survive
> Of course a proper (human) written documentation needs to be done once dust settles down
> Written by LLM, quick checked by me
...
| `@git` | Your git name and email, extracted from the host config and regenerated. Never bound. |
...
> **Alpha state
> A lot of details will change, be ready that your configuration won't survive
...
`-p @git` gives the sandbox the name and email you commit under, so
`git commit` works, without pinning anything else. It does **not** bind
`~/.gitconfig`:

```console
$ snug -p @git ~/src/proj -- cat '~/.gitconfig'
...
`@net` copies your host's address into the sandbox and drives your host's
resolver for DNS. Every address the host itself owns — including one `pasta`
never copies, the host's own link-local — is sealed as a local route inside the
sandbox's network namespace, so a connect to one is refused before it ever
leaves the sandbox.

The seal is also what makes the disclosure wider than one address: `pasta`
copies your default-route interface, the seal writes every host interface, so
`ip addr` inside lists your docker and podman bridges, `virbr0` and a VPN's
internal address alongside your LAN one. It names the networks your host is on;
it does not reach them. Accepted, and it is the price of the closure above.
...
bind a port — `scripts/payloads/http-door-server.py` is a working one. Servers you cannot
