...
1. **no root or dependnecies**: It does not require a `root`, `suid` helpers or an external daemon.
2. **Modern Linux capabilities**: Uses `bwrap`, `passta` and all modern Linux features
   like namespaces, the `at` family of syscalls and so to build the isolation.
3. **Hide the host by default**:  the host system is hidden by default - your `~/.ssh/` is not visible
   inside a sandbox.
4. **Integrates with host**: While `~/.ssh` is missing, `snug` can run an `ssh-agent` inside a sandbox with
   EXACTLY the keys needed for a job. It brings a concept of account to multi-account tools like `gh`.
4. **Declarative, easy to understand configuration**: TOML configuration and a
   concept of profiles, which are the Lego bricks building the sandbox itself. Additionally it
   tries to give user all information about how sandbox is going to be build.
5. **Avoids supply chain attacks**: Written in Go (with string nocgo policy) with a minimal set of 3rd party
   dependencies.
6. **Podman inside**: Runs own `podman` engine allowing docker-based workflows inside.
7. **Expose HTTP**: Can expose HTTP ports from a sandbox on demand via systemd socket activation protocol.
...
> I think that this says nothing about a code quality, however feel free to not use the tool
> in case you don't like the way it was written.
