...
|---|---|
| [`ENGINE-NETNS.md`](ENGINE-NETNS.md) | Why a container started through `@podman-socket` has the *engine's* network and not the sandbox's — and what moving the engine into the sandbox's netns costs. §0 is the canonical write-up of that finding, and carries the dated CLOSED banner for Tier B; [`TIER-B.md`](TIER-B.md) is what was decided about it. |
| [`TIER-B.md`](TIER-B.md) | The container engine in the sandbox's netns, as decided and measured (issue #63): the five settled maintainer questions, why the engine's capability bounding set is twelve and a constant rather than a profile field, why `CAP_NET_ADMIN` is excluded and `podman run -p` declined with it, the invariant-6 ruling and the hard Tier B / Tier C line — plus the measurement trap that makes a container's exit status a FALSE capability floor. |
...
| [`ENGINE-WIRING.md`](ENGINE-WIRING.md) | How the long-lived container engine composes with the one-shot stage: forked eagerly by P1 as a sibling of bwrap, `setns`'d into the sandbox's N, confined to twelve capabilities, its socket on `/tmp` because podman masks `/run`, and torn down on every path. The design pass behind `internal/stage`'s `__inengine` and `Stage.StartEngine`. |
| [`ATTACH.md`](ATTACH.md) | `snug attach`: which namespaces it joins and in what order, why the payload's user namespace rather than the mount-owning one, how the run's seccomp filter reaches a different process, the state file a run publishes about itself, and the measured stdio hole. Written before the code and left in that tense; §6.1 marks where what shipped differs. |
...
>
> **VERIFIED end to end at the time**, inside a distrobox container: `bwrap --json-status-fd 9` emitted `{ "child-pid": 59817, ... }`; `pasta --netns /proc/59817/ns/net --userns /proc/59817/ns/user --config-net ...` attached with rc=0; the sandbox then had a configured interface, working DNS, `curl https://example.com → 200`, and `127.0.0.1:631` / `127.0.0.1:3100` / `[::1]:3100` all refused. That measurement was correct when it was taken and is kept here only so a reader who meets the flags in an old comment can place them. **`--json-status-fd` is gone for good**, and `internal/sandbox/parked.go`, `readChildPID` and `waitForNetDevice` with it. `--block-fd` came back for an unrelated reason on container runs (issue [#125](https://github.com/gomoni/snug/issues/125), above) — never with the `--json-status-fd` half, never for the network, and never without `--sync-fd` on the same pipe.
>
...
**Status: shipped.** Tier B ([#63](https://github.com/gomoni/snug/issues/63)) landed on 2026-08-18 and was measured against a real engine. `@podman-socket` no longer includes `net`; the stage forks the container engine into **this sandbox's own N** (the engine leg of `internal/stage`'s `start` request and `__inengine`/`EnterEngine`, joining by `setns`; it was a `startengine` message of its own until issue #125's C2 gate folded it into `start`) and drops its capabilities to `policy.EngineCapBounding` — twelve, enumerated in `--dry-run`'s TOPOLOGY block, `CAP_NET_ADMIN` **not** among them.
...
- **Containers share N host-mode.** `HostConfig.NetworkMode = "host"` is the **one** namespace mode the proxy now allows, and Tier B inverted what it means: it says "join the engine's current netns", and the engine's current netns is N. Every other namespace mode stays refused unconditionally — `PidMode = "host"` above all. **The reason for that one changed under issue #125's C0 and the refusal did not.** Tier B's reason was that `__inengine` did not unshare pid, so the engine's pid namespace *was* the real host's; C0 gave the engine `CLONE_NEWPID` and a fresh procfs, so `PidMode = "host"` would now join the **engine's** pid namespace, not the host's (measured A/B against C0's parent commit). The refusal stays and is now conservative rather than the whole boundary: what it still declines is a namespace whose pid 1 is podman, root-in-U with `policy.EngineCapBounding`, and whose members include every other container's conmon — `/proc/<pid>/fd` and `/proc/<pid>/mem` there are not syscall-shaped (issue #47). Whether to relax it is **issue #145's** decision (`internal/dockerproxy/create.go`).
...
`-a`/`-g` (`@net-anon`, **one pair per family**, the prefix INLINE in `-a`) and `--mtu` are appended when the policy sets them. `-n` is never passed at all, in any configuration (issue #165): it is a single GLOBAL netmask, not per-family, and with it present an inline prefix in *either* `-a` is `Redundant prefix length specification` and `pasta` exits 1 — measured. Leaving a v6 prefix off works only because `pasta` defaults it to `/64`, a helper default deciding what the sandbox treats as on-link, which is exactly what this project refuses to inherit.

Deliberately **not** passed:

- `--map-guest-addr` — defaults to `none`, and `snug` wants no host→guest special address. Its absence is asserted behaviourally, not by trusting the default. Considered and rejected as a *partial* fix for `@net-anon`'s host-address reach (issue #176): it maps back at most one address per family, and a dual-stack host with two global v6 addresses (a stable one and a rotating privacy-extension one) left the second **REACHED** in the same run — a closure covering half a set invites the belief the set is closed.
- `-a`/`-g`/`-n` by default — `pasta` copies the host's addresses and routes into the namespace, so the sandbox sees the host's LAN address, **in both families** (**VERIFIED**: `192.168.1.120/24` and the host's global IPv6 addresses, temporary privacy-extension one included, all inside the ns). This is a real information disclosure, worse for v6 than v4 (globally routable and geolocatable vs. RFC1918). `snug` exposes withholding it as the opt-in `@net-anon` profile rather than a default, because overriding it diverges the sandbox's view from the host's and can confuse tooling. **VERIFIED** that `-a 10.13.13.2/24 -g 10.13.13.1 -a fd00:5e79:1::2/64 -g fd00:5e79:1::1` (no `-n`) works with egress intact on both families (`curl -4`/`curl -6` both `200`), and that `snug0` then carries neither of the host's own addresses, only the synthetic pair and a link-local whose EUI-64 derives from a per-run random tap MAC.
...
| Egress, **any profile that sets an address (`@net-anon`)**, host has a usable IPv4 resolver | `nameserver 169.254.1.1` | `pasta`, re-issuing to the v4 `--dns-host` |
...
The anonymising arms are issue #165 (families) built on issue #162's disclosure decision: `@net-anon` exists so the sandbox does not learn where the host sits; naming the host's LAN resolver on the next line of the same generated file gives it straight back, because a LAN resolver is normally the router and so discloses the prefix the synthetic address was hiding (and an IPv6 ULA resolver discloses a randomly-generated, stable per-site prefix). The condition is `Anonymised()` — true for **either** family's address being set, not only v4, and not only when both are — keyed off the field rather than the profile's name, so a future anonymising profile inherits it. IPv4 is preferred when the host has a usable one: the sandbox always has a v4 address and default route of its own, whereas on a v4-only host `pasta`'s v6 default is a local-mode stub, so a v4 forwarder is never stranded.

**VERIFIED** on this host, `@net-anon`, after the change: `cat /etc/resolv.conf` names `169.254.1.1` and neither `192.168.1.1` nor the host's ULA resolver; `getent hosts example.com` resolves; `curl -w %{http_code} https://example.com` returns `200`, three runs of three. **VERIFIED** separately on a fixture whose only resolver is IPv6: the file names `fd00:5e79:1::53`, and both lookups succeed there too.
...
- **Plain `resolv.conf` (this host, `nameserver 192.168.1.1`), when interception is in use at all — i.e. under `@net-anon`.** `--dns-host` is pinned explicitly to `192.168.1.1` rather than left to `pasta`'s own (non-family-aware) default. Queries go to `192.168.1.1` from the host netns. **VERIFIED** by a raw DNS query from inside the namespace: `DNS RESPONSE bytes=61 ancount=2`.
...
- **The sandbox's own address** is, by default, the host's, in BOTH families — `pasta` copies host addresses into the namespace. Stated plainly because it is a real information disclosure, worse for v6 (globally routable, geolocatable) than v4 (RFC1918): the agent learns your LAN IP and, without the v6 pair, your ISP-attributable global address too. `@net-anon` fixes both (issue #165) — and fixing it opens a documented trade (issue #176): the host's own address, no longer the sandbox's own, becomes reachable from inside (any service bound there, or on `0.0.0.0`/`::`), because the packet leaves the netns instead of being refused by the sandbox's own kernel. Host loopback stays closed regardless.
