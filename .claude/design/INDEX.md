...
| `@net` | topology (b) + the argv in §4.5, sealed per §4.5a | Full internet in/out. The sandbox's own address is copied from the host — a small, accepted disclosure. The host is unreachable on every address it owns, in both families, including its link-local (the seal, §4.5a) — not incidentally, by design. Host loopback unreachable. Host cannot reach sandbox ports. |
