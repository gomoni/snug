...
**This section once described topology A in the present tense, for a whole milestone, and no code had ever done it.** The consequence was live and user-visible: with `@podman-socket` selected and no `@net`, `--dry-run` printed "No egress" while a container reached the whole internet, because the engine runs on the host and its containers get the host's network.

**[`ENGINE-NETNS.md`](ENGINE-NETNS.md) is the truth on this subject** — the finding (§0), what a measured attempt at the inversion showed (§1–§2, it works), what it costs and where it does not work at all (§3, distrobox shims and subuid/cgroup delegation), which guarantees change shape (§4), and the two-stage plan (§5). Read it rather than reconstructing anything from here.

The one-line summary, so this section is not a bare pointer: an unprivileged process cannot create a bare netns, so the engine must join the *userns plus netns* that snug creates, keeping its own mount namespace — and the payoff, measured, is that container network becomes sandbox network in both directions and published ports land on the sandbox's loopback rather than the host's.
...
**Superseded.** Containers started through the proxied socket do not run in the sandbox's netns — they run in whatever netns the *engine* is in, which today is the host's. Every consequence of that, measured, plus what the proxy refuses in order to bound it, is in [`ENGINE-NETNS.md`](ENGINE-NETNS.md) §0 and §5; the endpoint filter itself is §7.2 here.

Two sentences that must not be lost: the sandbox cannot reach its own containers' published ports today, and a container's network posture is the *engine's*, not `snug`'s. The proxy's own refusals — `NetworkMode` of `host`/`container:*`/`ns:*`, `PortBindings`, `PublishAllPorts`, `Sysctls`, `DNS*`, `ExtraHosts`, and every host-namespace mode — are what keeps that bounded, and they are enumerated in §7.2.
...
Denied with `EPERM`: `ptrace`, `bpf`, `userfaultfd`, `add_key`, `keyctl`, `request_key`, `perf_event_open`; `ioctl(_, TIOCSTI, _)` (terminal input injection); `unshare`/`clone` with `CLONE_NEWUSER` (nested-userns escape primitive).
...
2. **Reject-list** the escape fields and the network fields: `HostConfig.NetworkMode` ∈ {`host`, `container:*`, `ns:*`}; `PortBindings`/`PublishAllPorts`; `Sysctls`, `DNS*`, `ExtraHosts`; `UsernsMode`, `PidMode`, `IpcMode`, `UTSMode`, `CgroupnsMode` set to any `host`/`container:` value; `Privileged`, `CapAdd`, `Devices`, `DeviceRequests`, `DeviceCgroupRules`, any `SecurityOpt`, custom `Runtime`, `Annotations` (podman honours `run.oci.*`), `VolumesFrom`, non-nil `MaskedPaths`/`ReadonlyPaths`.
...
Container-to-container networking on a `snug`-created bridge network **is** allowed, so multi-container workflows (app + database) work.
...
Read-only, from the host: the CLI itself (`~/.local/bin/claude`, `~/.local/share/claude`), `~/.claude/settings.json`, and `~/.claude/skills` + `~/.claude/plugins` re-exposed on top of the `~/.claude` tmpfs so they load and run normally.
...
Staging means the sandbox writes to a private copy on a tmpfs. **This has a real cost: a token refreshed inside the sandbox does not persist to the host.** `snug` handles it explicitly — at teardown it compares the staged copy to the host original and, if the sandbox wrote a *structurally valid* credentials file, copies it back atomically. Structural validation is the guard against a compromised agent writing arbitrary content into a host file. **`~/.claude.json` is never synced back**, because it carries MCP server configuration — a natural target for injecting a tool that runs *outside* the sandbox on your next host-side session.
...
**The writable surface is not one path.** The target bind is the only one that *persists*; `/tmp`, `$HOME`, `$HOME/.cache`, `$HOME/.config`, `$HOME/.local/state`, `$HOME/.local/share`, `$XDG_RUNTIME_DIR` and `/dev` are all writable tmpfs that die with the sandbox. Say "the only writable thing that persists", never "the only writable thing".
...
- **R4 — Credential sync-back (§9.3) writes to a host file from sandbox-authored bytes.** Structurally validated, but it is a real host-write channel from inside. Its scope is chosen so the sensitive-by-configuration file (`~/.claude.json`) is the one that never syncs.
