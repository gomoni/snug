...
**Re-verified 2026-08-23, from scratch, issue #384.** The table above is no
longer the only record of these numbers: `PROVENANCE` was regenerated by
downloading the pinned tag afresh and computing every value, because a
`PROVENANCE` whose sha256 is copied out of this document would cross-check the
bundle against a number of unknown origin — the "a value in prose is a copy of
state held somewhere else" shape, wearing the costume of an integrity check.
MEASURED: 33 784 113 bytes, sha256 `a58765fe…08d69`, both equal to the table;
`gpg --batch --verify` good, signing key `6E8EF2F5B1F26FF78431E3D1364FA5A62B410BA4`
under primary `0CCF102C4F95D89E583FF1D4F8B5AF50344BB503`, signature timestamp
unchanged. The circularity caveat above is unchanged and is carried into
`PROVENANCE` as an explicit `trust_is_circular = true` field, so a machine
reading that file cannot mistake a reproducibility claim for a trust one.

Then the check that makes the pin mean anything, which had never been made:
every binary in the install was compared against the same path extracted from
the verified tarball and matches **byte for byte** — `podman`
`3ae655eb…4d79`, `crun` `470318b2…990c`, `runc` `2a98cda9…730e`, `conmon`
`ae13d280…cd90`, `netavark` `54cb59da…f4d2`, `aardvark-dns` `81d1bae5…b8e1`.
The engine on this host is provably the pinned artifact.

**Why this is still 5.8.4 and not podman 6.** MEASURED 2026-08-23 — there is no
6.x **static local-engine** bundle to pin, from either supplier. This project's
newest tag is `v5.8.4` and it has no 6.x tag at all; its podman-6 pull request
(`mgoltzsche/podman-static#168`, "feat: podman 6.0.0") is open and unmerged.
Upstream `containers/podman` does publish 6.x (`v6.1.0`, 2026-08-12) but its
only linux assets are `podman-remote-static-linux_{amd64,arm64}.tar.gz` — the
remote **client**, with no `conmon`, `crun` or `netavark`, so it cannot serve
§6's question at all. The only podman 6.x reachable here is `/usr/bin/podman`
(6.0.2), which is a symlink to `distrobox-host-exec`: the shim `podmanBundle`
and preflight P1 exist to refuse. A 6.x pin therefore needs a **new supplier**,
which reopens the trust decision this section makes knowingly — a maintainer's
call, not a version bump. Deferred; the pin lives in one constant
(`engine.PinnedPodmanVersion`) so the bump is a one-line change when #168 lands.
...
**READ THAT AS THE ORIGINAL INSTALL, not as a description of your disk — and
this section is written in the present indicative, which is the trap.** There is
no provisioning script anywhere in the tree: `git ls-tree -r` matches nothing
for provision/bootstrap/install outside this document, so the layout above is a
checklist a human re-runs from memory, and 2026-08-23 measured what that costs.

MEASURED, issue #384, on the development host ten days after this section was
written: `~/.local/opt/podman-static/` held `README.md  usr/  etc/` **and
nothing else** — `PROVENANCE`, the `.asc`, `bin/snug-podman*`, `etc/snug/`,
`home/` and `var/` all absent. The vendor half is faithful (§1's byte-for-byte
comparison); it is snug's own layer that is not there.

**What the timestamps establish, and what they do not.** btrfs records birth
time, so this can be settled rather than argued:

```
$ stat -c '%w  |  %n' ~ ~/.local ~/.local/opt ~/.local/opt/podman-static
2026-02-11 16:26:42.873199236 +0100  |  /home/michal
2026-02-11 16:26:42.874199236 +0100  |  /home/michal/.local
2026-08-22 16:59:35.868690118 +0200  |  /home/michal/.local/opt
2026-08-22 16:59:35.868690118 +0200  |  /home/michal/.local/opt/podman-static
```

`~/.local/opt` and the bundle directory were **born at the same nanosecond** —
one `mkdir -p`, 2026-08-22 — so the directory holding this install has no
predecessor on this filesystem. The home is **not** fresh: `~` and `~/.local`
were born 2026-02-11, six months earlier. And a trap worth naming because it is
what a first pass concludes: `~/.local` and `~/.local/opt` share an *mtime* to
the nanosecond, which looks like proof they were created together and is not —
creating `opt` stamps its parent's mtime. **Directory mtime is not birth time.**

So two readings both fit, and this section deliberately picks neither: either an
earlier full install existed here and was removed before 2026-08-22 (a deleted
inode's birth time is unrecoverable, so it cannot be excluded), or the full
install above was never made on this machine and §3 records one made elsewhere.
§3's own "MEASURED before writing: `~/.local/opt` did not exist" dates its
install to 2026-08-12, and the present `~/.local/opt` was born 2026-08-22, so
§3's directory is not the one on disk under either reading.

**The point that survives both readings is the whole of #384, and it is why this
matters more than the archaeology.** `podmanBundle` gated the entire engine
suite on `os.Stat` of the binary plus `!IsDir()`, and `preflightPodmanBinary`
trusted `$SNUG_PODMAN` on the same two checks — so **a bundle provisioned fresh,
a bundle re-provisioned at the same tag, and a bundle re-provisioned at a
DIFFERENT tag are all indistinguishable to the gate.** Nothing about the six
missing entries went red, and nothing about a wrong version would have either.
The gate now execs `--version`, compares it against
`engine.PinnedPodmanBundleVersion`, and cross-checks `PROVENANCE`; a *mismatch*
is a `t.Fatal`. `t.Skip` means "no bundle" and nothing else, because skipping on
a wrong bundle preserves exactly the silence this paragraph describes.

**`bin/snug-podman*` were deliberately NOT rebuilt**, and that is not laziness
about a checklist. The committed suite does not use them: see
`containerEngineEnv` in `test/integration/containerengine_test.go` — "NO WRAPPER
SCRIPT, and its removal is a simplification Tier C forced" — since everything
the wrapper did is snug's own job now (#212 `storage.conf`, #133
`containers.conf`, preflight P5's `cgroups=disabled`). Only `PROVENANCE` and the
`.asc` were restored, being what the version gate reads. Note the reference
outlives the file: `internal/cli/containerpreflight.go` still cites
`bin/snug-podman` as the precedent for the `$SNUG_PODMAN_ROOT` vocabulary.
...
reason. MEASURED — and **corrected 2026-08-23, issue #384: this said "no
`--signature-policy` flag" and that is false.** The flag is *per-subcommand*.
Re-measured against the pinned binary, classifying on whether cobra rejects the
flag by name (control: `podman pull --snug-bogus-flag=1` → `Error: unknown flag:
--snug-bogus-flag`):
...
So it is **absent from the global command and from `system service`, and hidden
but present on the image-consuming subcommands** — which is what
`SIGNATURE-POLICY.md` §1 already recorded for `pull` and `push`, this
re-measurement merely adding `create`, `run` and `build`. **The conclusion below
is untouched, and the rejection is why:** `system service` is the only command
snug invokes, the container proxy's pull happens inside that process over the
API, and no per-command flag reaches it. The private `HOME` really is the whole
mechanism. Meanwhile containers/image looks for a signature policy at exactly
two paths:
...
6. **podman 5.8.4's `--signature-policy` does not reach `system service`.**
   MEASURED, and **restated 2026-08-23 (issue #384): this item used to claim the
   flag did not exist at all, which is false** — it is hidden but accepted on
   `pull`, `image pull`, `create`, `run` and `build`, and rejected with `Error:
   unknown flag` on the global command and on `system service`. Since `system
   service` is the only command snug runs, the effect is the same and the
   workaround is unchanged: a private `HOME` (§5). If a future bundle changes
   where it looks for `policy.json`, this breaks with a message about signatures
   rather than about `HOME`.
