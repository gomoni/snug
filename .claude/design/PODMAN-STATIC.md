...
**The source tree is the authority for this tuple, not this table.**
`engine.SupportedPodmanBundles` (`internal/engine/podmanpin.go`) holds tag,
version, tarball sha256 and size, and is the only supported set — the numbers
above are a reader's copy of it. The gate compares an installed bundle's
`podman --version` against that set; `PROVENANCE` is cross-checked against it
too.

Every installed binary is byte-identical to the same path in the verified
tarball: `podman` `3ae655eb…4d79`, `crun` `470318b2…990c`, `runc`
`2a98cda9…730e`, `conmon` `ae13d280…cd90`, `netavark` `54cb59da…f4d2`,
`aardvark-dns` `81d1bae5…b8e1`.

`PROVENANCE` carries the same tuple plus per-binary hashes, and records the
circularity above as `trust_is_circular = true` — the caveat is machine-readable
rather than prose-only.

**5.8.4 is the supported version, by maintainer's decision, 2026-08-23.** No
6.x static local-engine bundle is published: this project has no 6.x tag, and
upstream `containers/podman` ships only `podman-remote-static-*` for linux, a
remote client with no `conmon`, `crun` or `netavark`. `/usr/bin/podman` on a
distrobox host is `distrobox-host-exec`, which preflight P1 refuses. A later
version becomes a second member of `engine.SupportedPodmanBundles`.

Install: `README.md`, "Containers, and the `podman` shim".
...
**The tree above is the full provisioning; a plain `tar -xzf … --strip-components=1`
produces only `README.md`, `usr/` and `etc/`.** There is no provisioning script
— `git ls-tree -r` matches nothing for provision/bootstrap/install outside this
document — so the rest is added by hand, and a bundle may legitimately be found
without it.

The gate therefore reads `os.Stat` + `podman --version` and nothing else:
absent is a skip, present-but-unsupported is a failure. `PROVENANCE` and the
`.asc` are cross-checked when present and skipped when not, so the documented
minimal install is not punished for following this section.

`bin/snug-podman*` are not needed by the committed suite: snug generates
`containers.conf`, `storage.conf` and `registries.conf` itself and selects
`cgroups=disabled` in preflight P5. `internal/cli/containerpreflight.go` still
cites `bin/snug-podman` as the origin of the `$SNUG_PODMAN_ROOT` name.

**Nothing outside this root is written.** The install owns
`~/.local/opt/podman-static` and a runtime directory `/tmp/snug-podman-static/`
(§5); `~/.config/containers/` and `/etc/containers/` are left untouched, and no
`~/.config/containers/policy.json` is created — the engine gets its own `HOME`
instead (§5).
...
reason. `--signature-policy` is PER-SUBCOMMAND. MEASURED, classifying on
whether cobra rejects the flag by name (control: `--snug-bogus-flag` ->
`Error: unknown flag: --snug-bogus-flag`):

```
podman --signature-policy=/x version           Error: unknown flag: --signature-policy
podman system service --signature-policy=/x …  Error: unknown flag: --signature-policy
podman pull|image pull|create|run|build …      accepted (hidden, absent from --help)
```

Rejected on the global command and on `system service`, hidden but accepted on
the image subcommands. `system service` is the only command snug runs and the
proxy's pull happens inside it over the API, so no per-command flag reaches it
and the private `HOME` is the whole mechanism. containers/image looks for a
signature policy at exactly two paths:
...
6. **`--signature-policy` does not reach `system service`.** MEASURED: rejected
   with `Error: unknown flag` on the global command and on `system service`,
   hidden but accepted on `pull`, `image pull`, `create`, `run` and `build`.
   `system service` is the only command snug runs, so the lever is a private
   `HOME` (§5). If a future bundle changes where it looks for `policy.json`,
   this breaks with a message about signatures rather than about `HOME`.
