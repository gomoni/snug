...
//
// cmd/snug is the right home for both: it sits above internal/policy, so it can
// see the writers on both sides of the Resolve boundary. go/parser is stdlib,
// so this costs no dependency and internal/policy stays pure.
...
func TestSnugOwnedEnvIsExactlyWhatSnugWrites(t *testing.T) {
	...
	for _, dir := range []string{filepath.Join("..", "..", "internal", "policy"), "."} {
