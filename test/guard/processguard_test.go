...
// THE KILL RULE IS UNIVERSAL, WHICH IS WHY IT IS NOT THE SANDBOX-RUN RULE.
//
// TestOnlyRedteamCarriesTheSandboxRunRule is exact on purpose: running on the
// host is the ordinary and correct mode for every agent except redteam, so a
// block warning them about it spends attention where nothing is at stake.
//
// This rule is the opposite shape. `pkill -x bwrap` is exactly as destructive
// from go-implementer's shell as from redteam's, and neither of them has to be
// running a sandbox for it to land. So every agent file whose tools include
// Bash and whose ordinary work starts processes carries it.
const killRuleHeading = "## Killing a process, and commands that pick the target for you"

func TestEveryAgentThatStartsProcessesCarriesTheKillRule(t *testing.T) {
	files := agentFiles(t)

	for _, name := range []string{"redteam.md", "sandbox-tester.md", "host-bridge.md", "go-implementer.md"} {
		body, ok := files[name]
		if !ok {
			t.Errorf("%s is missing from .claude/agents", name)
			continue
		}
		if !strings.Contains(body, killRuleHeading) {
			t.Errorf("%s does not carry %q. Every agent here has a Bash tool, and a host shell "+
				"kills host processes (issue #197)", name, killRuleHeading)
			continue
		}
		// Asserted as content rather than as a heading, because a heading with
		// the body rewritten under it is what a well-meaning edit produces.
		for _, want := range []struct{ text, why string }{
			...
			if !strings.Contains(body, want.text) {
				t.Errorf("%s's kill-rule section no longer says %q — %s", name, want.text, want.why)
			}
		}
	}
}
