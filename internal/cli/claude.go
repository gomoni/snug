...
func claudeStateJSON(pol *policy.Policy) []byte {
	b, err := json.MarshalIndent(claudeState(pol), "", "  ")
	if err != nil {
		return nil
	}
	return append(b, '\n')
...
// claudeState builds the key set claudeStateJSON marshals, split out so the
// injected guidance can NAME those keys through claudeStateNames instead of
// counting them in prose. Adding a key here is the policy change
// claudeStateJSON's doc comment describes, and it now reaches the agent-facing
// screen without a second edit.
func claudeState(pol *policy.Policy) map[string]any {
...
// claudeStateNames renders claudeState's top-level keys as a prose list, in the
// order they appear in the generated file: json.MarshalIndent sorts map keys,
// so sorting here is what makes the guidance's list and the file's byte order
// the same sequence for a reader comparing them.
func claudeStateNames(pol *policy.Policy) string {
	names := make([]string, 0, len(claudeState(pol)))
...
func claudeGuidance(pol *policy.Policy) []byte {
	...
	// ENUMERATED, for the reason claudeAllowlistNames exists a few paragraphs
	// down: this sentence hand-counted claudeState's keys ("at most three
	// keys"), which is a copy of that table's state, and it was ALSO checkably
	// false to its own reader — the file is on the writable $HOME tmpfs and
	// Claude Code rewrites it. Measured, claude 2.1.263 in a real snug run:
	// 37 top-level keys two minutes into a session and 40 a moment later, with
	// `autoUpdates` — one of the three snug writes — gone from the file.
	fmt.Fprintf(&b, "`~/.claude.json` here is GENERATED by snug, which writes %s. The\n",
		claudeStateNames(pol))
	...
	// The DROP is the invariant and is stated as one: the allowlist is the
	// filter, so "everything else" is the whole truth and the named keys are
	// examples of it. The previous phrasing ("NOT carried:" followed by five
	// names) read as an exhaustive list, which made a key's PRESENCE in the
	// file look like evidence snug had carried it — see the rewrite paragraph
	// below for the `env` case that actually happens. Enumerating the real
	// refused set is not the fix: policy.ClaudeExecutingKeys catalogues 91
	// names, and this is text an agent pays for on every read.
	// TestClaudeGuidanceNamesOnlyRefusedKeys pins every name below to that
	// catalogue, so an example that stops being refused cannot survive here.
	fmt.Fprintf(&b, "host's. Carried: %s, and NOTHING\n", claudeAllowlistNames())
	b.WriteString("else — every other key in the host's file is dropped, among them\n")
	...
	b.WriteString("(empty by default), so Claude Code loads only those; and this settings file\n")
	b.WriteString("drops `enabledPlugins`, a second gate that stops any of them firing here at\n")
	b.WriteString("all (measured, claude 2.1.238). A test drives the real binary against the\n")
	b.WriteString("regenerated manifest and asserts an unnamed plugin's hook does not fire; see\n")
	...
	// THE MISSING TENSE, and the reason the two paragraphs above were read as
	// false from inside. Both files are writable tmpfs files owned by the
	// sandboxed process, and Claude Code rewrites both AFTER snug hands off.
	// Measured in a real run, claude 2.1.263, from /proc/2/stat field 22
	// against stat(1) mtimes: this guidance file lands 25 ms BEFORE claude
	// starts, ~/.claude/settings.json 247 ms after it, ~/.claude.json 51 s
	// after it. So the guidance cannot describe their content — only what snug
	// wrote. Both instances are real and both were measured on that run:
	// `autoUpdates` was dropped from ~/.claude.json, and `env` appeared in
	// settings.json (Claude Code disabling its own auto-updater, which can
	// only fail against a read-only bind at policy.StagedBinDir). NEITHER
	// INSTANCE IS NAMED ON THE SCREEN: they are one claude version's
	// behaviour, and prose here that copied them would drift the next time
	// that behaviour changed. The invariant is what an agent needs.
	b.WriteString("BOTH FILES ABOVE ARE REWRITTEN BY CLAUDE CODE once it starts, so their\n")
	b.WriteString("CONTENT is its state and not snug's: a key snug wrote may be gone, and a\n")
	b.WriteString("key snug never carried may be present. What reached you from the host is\n")
	b.WriteString("the `Carried:` list above and nothing else — reading either file is not\n")
	b.WriteString("evidence either way, and a key you find in one is not a hole to report.\n\n")
