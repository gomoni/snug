...
// sshConfigChain asks this host's ssh which configuration files it reads and
// returns the system-wide ones, so snug can replace a spelling nobody wrote
// down (issue #42).
//
...
func sshConfigChain(home string, verbose bool) []string {
	...
	if err := cmd.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "snug: could not ask ssh which configuration files it reads (%v), "+
			"so snug falls back to the two spellings it knows (%s).\n"+
			"      On a host that spells it a third way, ssh inside the sandbox will fail with "+
