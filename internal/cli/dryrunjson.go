...
// jsonBwrap.Argv[0] is the literal string "bwrap", PREPENDED to
// Report.BwrapArgv (refs #332, the mechanical half). Before this the key
// named `argv` held only the ARGUMENTS — index 0 was "--unshare-user" — so
// `subprocess.run(doc.bwrap.argv)` executed a flag. It is the literal word,
// NOT exec.LookPath's resolved path: a resolved path is host-dependent and
// would put a per-developer value into the committed golden fixtures, the
// trap issues #301 and #320 both refused.
...
//
// Argv[0] is the literal string "pasta", prepended the same way and for the
// same reason as jsonBwrap.Argv[0]: Report.Pasta.Argv holds only the
// arguments (index 0 is "--config-net"), and a key named `argv` should not
// hold a value whose first element is a flag.
...
func (e *lossyEncoder) document(rep Report) jsonDoc {
	...
	// "bwrap", the literal word, PREPENDED — never exec.LookPath's resolved
	// path (host-dependent, would leak a per-developer value into the golden
	// fixtures). See jsonBwrap's doc comment.
	bwrapArgv := make([]string, 0, len(rep.BwrapArgv)+1)
	bwrapArgv = append(bwrapArgv, "bwrap")
	...
	if pa := rep.Pasta; pa != nil {
		...
		pastaArgv = append(pastaArgv, "pasta")
		pastaArgv = append(pastaArgv, pa.Argv...)
		jp := jsonPasta{Placeholder: pa.Placeholder}
