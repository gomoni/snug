...
// imports internal/policy — internal/policy itself, internal/cli,
...
// structTable maps package label ("policy", "cli", "sandbox", ...) -> type
// name -> struct AST. One label per swept directory, all in one table,
// because e.g. an internal/cli field can be typed `policy.Mount` and resolving
...
// with (internal/policy and internal/cli). internal/sandbox iterates
...
func loadProductionStructTables(t *testing.T) structTable {
	t.Helper()
	tables := structTable{}
	collectStructTypes(t, filepath.Join("..", "policy"), "policy", tables)
	collectStructTypes(t, ".", "cli", tables)
