...
//
// # What each of the proxy's jobs guards, and whether it is the only thing standing there
//
// Tier C (issue #125) gave the engine a mount view DERIVED from the sandbox's
// and a pid namespace of its own; that turned some of these decisions into
// defence in depth and left the rest as the sole guard. The split is per job —
// per KEY, in one row — and reading a belt-and-braces row as load-bearing is
// how a real gap gets ranked below a cosmetic one.
//
...
// | **run label** — `stampRunLabel` (`create.go`), read by the ownership gate (`ownership.go`) | one run's containers from every earlier run's, in a store that outlives both | **Sole guard.** The store is keyed on the target and persists, and teardown removes no container record. The gate asks the engine for the label of the container a route addresses and forwards re-addressed to the immutable 64-hex id, which is the structural close for the rename race measured in issue #386. |
